...
# Data.of("obel1sk.maxQuickReloadWindow"): maximum amount of seconds the quickload can be there if the span of min - max is too large
...
#const RETICLE_TARGET = preload("res://mods-unpacked/Snek-Phosphorescence/overwrites/content/weapons/obelisk/ReticleTarget.tscn")
const LIGHTNING_UP = preload("res://mods-unpacked/Snek-Phosphorescence/overwrites/content/weapons/obelisk/LightningUp.tscn")
const ARC = preload("res://mods-unpacked/Snek-Phosphorescence/overwrites/content/weapons/obelisk/ObeliskShockArc.tscn")
const KILLSTREAK = preload("res://mods-unpacked/Snek-Phosphorescence/overwrites/content/weapons/obelisk/KillstreakEffect.tscn")
const RAD_AREA = preload("res://mods-unpacked/Snek-Phosphorescence/overwrites/content/weapons/obelisk/RadiationApplyArea.tscn")
const DEATH_CANDLE = preload("res://mods-unpacked/Snek-Phosphorescence/overwrites/content/weapons/obelisk/DeathCandle.tscn")
...
func init(_unusedCupolaPath):
	...
	Data.listen(self, "obel1sk.maxAmmo", true)
	Data.listen(self, "obel1sk.reloadTime", true)
	Data.listen(self, "obel1sk.minQuickReload", true)
	Data.listen(self, "obel1sk.maxQuickReload", true)
	Data.listen(self, "obel1sk.maxQuickReloadWindow", true)
	Data.listen(self, "obel1sk.attackType", true)
	Data.listen(self, "obel1sk.arcCooldown", true)
	Data.listen(self, "monsters.wavepresent", true)
	Data.listen(self, "obel1sk.shotType", true)
	Data.listen(self, "obel1sk.markCurrent", true)
	Data.listen(self, "obel1sk.killstreaks", true)
	Data.listen(self, "obel1sk.chStyle", true)
	Data.listen(self, "obel1sk.mercilessMax", true)
	Data.listen(self, "obel1sk.shootDelay", true)
	Data.listen(self, "obel1sk.autoAimDuration", true)
	Data.listen(self, "dome.health")
	Data.listen(self, "obel1sk.storedDamageMax", true)
	Data.listen(self, "obel1sk.damage", true)
	
	maxAmmo = int(Data.of("obel1sk.maxAmmo"))
	attack_type = int(Data.of("obel1sk.attackType"))
	arcCooldown = Data.of("obel1sk.arcCooldown")
...
func move(dir:Vector2, allowMove:bool = true):
	...
	if is_reloading and (not Data.of("obel1sk.killstreakActiveFullSpeed") or not $KillstreakTracker.active()) and not Data.of("obel1sk.rechargeAmmo"):
		speed_mod *= Data.of("obel1sk.speedWhileReloading")
	if Data.of("obel1sk.killstreaks") and Data.of("obel1sk.killstreakInactiveEffects"):
		if not $KillstreakTracker.active() and $KillstreakTracker.progress >= $KillstreakTracker.Goal() * Data.of("obel1sk.killstreakGHFthreshold"):
			speed_mod *= Data.of("obel1sk.killstreakInactiveSpeed")
	
	$Reticle.killstreakActive = $KillstreakTracker.active()
	
	if cur_ammo <= 0 and not is_reloading: speed_mod *= Data.of("obel1sk.speedWhileNoAmmo")
	...
	match attack_type:
		AttackTypes.Shot:
			if (($Reticle.cur_spread > $Reticle.maxSpread() + cur_shootingSpread * 0.1) and cur_ammo > 0) or isShootingA:
				if not Data.of("obel1sk.killstreakActiveFullSpeed") and not $KillstreakTracker.active():
					speed_mod *= Data.of("obel1sk.speedWhileShooting")
				isShootingM = true
			else:
				isShootingM = false
		...
		AttackTypes.Beam:
			if (is_beam_active()):
				if not Data.of("obel1sk.killstreakActiveFullSpeed") and not $KillstreakTracker.active():
					...
					speed_mod *= Data.of("obel1sk.speedWhileShooting") * t
			dir.y = 0
	...
	if is_instance_valid(beam):
		...
		if not $BeamTargetPointer.is_colliding() or Data.of("obel1sk.piercingBeam"):
...
func action(fireStrength:float, specialStrength:float, allowShoot:bool):
	...
	if fire < last_fire:
		fire = 0.0
	if spec < last_spec or (int(Data.of("obel1sk.ammoUsage")) == 0 and Data.of("obel1sk.shotType") != 1):
		spec = 0.0
	...
	if not shoot_button_down_from_reload:
		match attack_type:
			AttackTypes.Shot:
				match int(Data.of("obel1sk.shotType")):
					ShotTypes.Explosion:
						if not int(Data.of("obel1sk.specialType")) == SpecialTypes.SlowReticle:
	...
	if fire > 0.0 and not Data.of("obel1sk.allowReload") and cur_ammo == 0 and not shoot_button_down_from_shot and not Data.of("obel1sk.fullAuto") and timeSinceLastShot > 0.1:
		$Reticle.denial()
	
	match int(Data.of("obel1sk.specialType")):
		SpecialTypes.Reload:
	...
	if fire > 0.0 and Data.of("obel1sk.rechargeAmmo") and is_reloading and not reloadFromEmpty:
...
func actionF_shot(fire, spec):
	if fire > 0.0 and can_shoot and cur_ammo > 0 and not shoot_button_down_from_reload and timeSinceWaveEnded > 1.0 and ((not is_reloading and not Data.of("obel1sk.rechargeAmmo")) or Data.of("obel1sk.rechargeAmmo")):
		if not Data.of("obel1sk.fullAuto"):
...
func actionS_reload(fire, spec, secondaryAction:=false):
	...
	if (spec_quick or fire_quick) and Data.of("obel1sk.maxQuickReloadWindow") > 0.0 and Data.of("obel1sk.allowReload"):
		if quick_reload_attempts <= 0 and cur_reloadTime > anticipationDeadzone():
			...
			if p >= _min * 0.95 and p <= _max * 1.15: # factors are coyote time
				if fire_quick:
					...
					shoot_button_down_from_reload = Data.of("obel1sk.chStyle") != 4
				reloadFillAmmoQuick()
	...
	if spec > 0.0 and can_reload and not is_reloading and Data.of("obel1sk.allowReload") and Data.of("obel1sk.allowManualReloadStart"):
		reloadStart()
	...
	if fire > 0.0 and not Data.of("obel1sk.autoReload") and cur_ammo <= 0 and not shoot_button_down_from_shot and not is_reloading and Data.of("obel1sk.allowReload")and Data.of("obel1sk.allowManualReloadStart"):
		reloadStart()
	
	
	if fire > 0.0 and not shoot_button_down_from_shot and not shoot_button_down_from_reload and last_fire != fire and Data.of("obel1sk.allowReload") and timeSinceLastShot > 0.1:
		if cur_reloadTime > 0.0 and quick_reload_attempts > 2:
			$Reticle.denial()
		elif cur_shootDelay > 0.0 and cur_shootDelay < shootDelay() and not Data.of("obel1sk.fullAuto"):
			$Reticle.denial()
	
	if not Data.of("obel1sk.fullAuto") and fire == 0.0:
		shoot_button_down_from_shot = false
	...
	if spec > 0.0:
		if can_detonate:
			...
			while marks.size() > 0 and inf_safety < 40:
				...
				inf_safety += 1
			Data.apply("obel1sk.markCurrent", 0)
...
func actionS_slow(fire, spec):
	if spec > 0.0:
		special_slow = Data.of("obel1sk.speedWhileSpecialSlow")
...
func propertyChanged(property:String, oldValue, newValue):
	match property:
		...
		"obel1sk.minquickreload": 
			init_quick_reload()
		"obel1sk.maxquickreload": 
			init_quick_reload()
		"obel1sk.maxquickreloadwindow": 
			init_quick_reload()
		"obel1sk.attacktype":
		...
		"monsters.wavepresent":
			if oldValue != newValue:
				...
				if not newValue:
					...
					$MercilessTracker.set_bonus(Data.of("obel1sk.mercilessBase"), false)
...
func _process(delta: float) -> void:
	...
	if Data.of("obel1sk.chStyle") == 5:
		decay_threshold = Data.of("obel1sk.shootDelay") + 0.025
	if $Reticle.shooting_spread != 0 and (timeSinceLastShot > decay_threshold):
		var decay = Data.of("obel1sk.shootingSpreadDecay")
		var growth = Data.of("obel1sk.shootingSpreadGrowth")
		
		if abs(cur_shootingSpread) <= growth * growth and int(Data.of("obel1sk.chStyle")) == 1:
			if decay > 0:
				decay = sqrt(sqrt(decay))
		var decayed = cur_shootingSpread
		if Data.of("obel1sk.shootingSpreadGrowth") >= 0:
	...
	if int(Data.of("obel1sk.burstCount")) == 1:
		...
		if cur_burstCount == 0:
			...
			if cur_burstDelay < Data.of("obel1sk.burstDelay"):
				cur_burstDelay += delta
			else:
				if cur_burstCount > 0 and cur_burstCount < int(Data.of("obel1sk.burstCount")):
					
					shoot()
					
					#cur_burstDelay = 0.0
				elif cur_burstCount >= int(Data.of("obel1sk.burstCount")):
					if cur_shootDelay < shootDelay():
						cur_shootDelay += delta
					else:
						if not $ReadyLightning.playing and not can_shoot and not is_reloading:
	...
	if isShootingA and reloadFromEmpty:
		what = true
	if what and Data.of("obel1sk.rechargeAmmo"):
		var delay = Data.of("obel1sk.batteryDelay")
		if $KillstreakTracker.active():
			delay *= Data.of("obel1sk.killstreakActiveReloadMultiplier")
		if $MercilessTracker.aboveThreshold():
			delay *= Data.of("obel1sk.mercilessThresholdReloadMultiplier")
		if cur_batteryDelay < delay:
			cur_batteryDelay += delta
		else:
			var charge = max((1.0 / reloadTime()) * maxAmmo * delta, 0.01)
			if $KillstreakTracker.active():
				charge *= 1.0 / Data.of("obel1sk.killstreakActiveReloadMultiplier")
			if $MercilessTracker.aboveThreshold():
				charge *= 1.0 / Data.of("obel1sk.mercilessThresholdReloadMultiplier")
			...
			if cur_batterySingleProgress >= 1.0:
				...
				if last_a != cur_ammo and Data.of("obel1sk.adStyle") == 0:
					$AmmoRecharge.pitch_scale = 0.6 if reloadFromEmpty else 1.0
					$AmmoRecharge.play(0.0)
		
		if cur_ammo < Data.of("obel1sk.maxAmmo") and not is_reloading and cur_batteryDelay >= delay:
			ammoRatioOnReloadStart = float(cur_ammo) / float(Data.of("obel1sk.maxAmmo"))
			init_quick_reload()
			reloadStart()
			cur_batterySingleProgress = 0.0
	
	if not Data.of("obel1sk.rechargeAmmo"):
		...
		if cur_reloadTime < reloadTime(true) and cur_reloadTime > 0.0:
			cur_reloadTime += delta
		if cur_ammo >= Data.of("obel1sk.maxAmmo") and is_reloading:
	...
	if attack_type == AttackTypes.Beam and is_beam_active():
		if cur_beamAmmoTickDelay < beamAmmoTickDelay:
			...
			set_cur_ammo(cur_ammo - int(Data.of("obel1sk.ammoUsage")))
			if Data.of("obel1sk.autoReload") and cur_ammo <= 0 and Data.of("obel1sk.allowReload") and Data.of("obel1sk.enterAutoReloadOnLastAmmo"):
				reloadStart()
	...
	if not is_reloading and ($Reticle.input == Vector2.ZERO or spreadDeactivated() or (Options.useMouseDomeGameplay and $Reticle.position.distance_to(get_local_mouse_position()) < 0.2)) and is_active:
		...
		if Data.of("obel1sk.specialType") == 2 and special_slow == Data.of("obel1sk.speedWhileSpecialSlow") and cur_ammo > 0:
			...
			if cur_special_slow_coyote_time < special_slow_coyote_time:
				...
				var reduction = cur_restDamageMult * 0.5 * Data.of("obel1sk.restDamageDecayMult")
				setCurRestDamageMult(cur_restDamageMult -reduction)
				if Data.of("obel1sk.maxRestDamageMult") != 0:
					if Data.of("obel1sk.maxRestDamageMult") < 0.05 and cur_ammo > 0:
	...
	if not is_shooting() and Data.of("obel1sk.maxShootDelayReduction") > 0.0:
		cur_shoot_delay_reduction -= (shoot_delay_reduction_decay / Data.of("obel1sk.maxShootDelayReduction")) * delta
		cur_shoot_delay_reduction = max(cur_shoot_delay_reduction, 0)
	
	if $KillstreakTracker.active() and Data.of("obel1sk.killstreakActiveEffects"):
	...
	if Data.of("obel1sk.chStyle") == 4:
		$BeamOutline.visible = canBeamOutlineBeVisible()
		$BeamOutline.clear_points()
		var width = Data.of("obel1sk.beamWidth") / 2.0
	...
	if timeSinceLastShot < 0.5:
		for m in arcStunMonsters:
			if is_instance_valid(m):
				m.hit(0, Data.of("obel1sk.StunAlongArc"))
...
func build_up_rest_damage_mult(delta, builup_fac:=1.0):
	...
	setCurRestDamageMult(clamp(cur_restDamageMult, 0, Data.of("obel1sk.maxRestDamageMult")))

func spreadDeactivated():
	if Data.of("obel1sk.killstreakActiveNoSpread") and $KillstreakTracker.active():
		return true
	
	if noSpreadRemainingDuration > 0.0:
		return true
	
	if $MercilessTracker.aboveThreshold() and Data.of("obel1sk.mercilessThresholdNoSpread"):
		return true
	
	return false

func get_reload_progress():
	var p = max(0, cur_reloadTime - anticipationDeadzone()) / reloadTime(Data.of("obel1sk.rechargeAmmo"))
	p *= 1.0 + anticipationDeadzone() / reloadTime(Data.of("obel1sk.rechargeAmmo"))
	return p

func anticipationDeadzone():
	if Data.of("obel1sk.rechargeAmmo"):
		return 0
...
func merciless_tracker_visible(value:bool):
	$MercilessTracker.visible = value and Data.of("obel1sk.mercilessMax") > 0

func ammo_counter_visible(value: bool):
	$AmmoDisplay.visible = (value and int(Data.of("obel1sk.ammoUsage")) > 0) or Data.of("obel1sk.shotType") == 1

func bonus_damage_visible(value:bool):
	if not Data.of("obel1sk.maxRestDamageMult") > 1.0 or not Data.of("obel1sk.fullRestDamageAdd") > 0:
...
func init_quick_reload():
	var _min = Data.of("obel1sk.minQuickReload")
	var _max = Data.of("obel1sk.maxQuickReload")
	#print(str((_max - _min) * reloadTime, " ", reloadTime))
	if (_max - _min) * reloadTime(Data.of("obel1sk.rechargeAmmo")) > Data.of("obel1sk.maxQuickReloadWindow"):
		var max_span = _min + (Data.of("obel1sk.maxQuickReloadWindow") / reloadTime(Data.of("obel1sk.rechargeAmmo")))
		_max = max_span
		#maxQuickReload = _max
...
func set_can_shoot(value:bool):
	if not can_shoot and value and Data.of("obel1sk.chStyle") != 5 and not is_reloading:
		if ((Data.of("obel1sk.fullAuto") and not isShootingA) or not Data.of("obel1sk.fullAuto")) and is_active and shootDelay() > 0.2:
...
func shoot(
	...
	if GameWorld.paused or not is_active:
		return
	var shot_type_override=int(Data.of("obel1sk.shotType"))
	var pos_override:=shot_pos()
	var single_target_override=Data.of("obel1sk.singleTarget")
	var ammo_usage_override=int(Data.of("obel1sk.ammoUsage"))
	var radius_override=Data.of("obel1sk.radius")
	
	if int(Data.of("obel1sk.chStyle")) == 1 and timeSinceLastShot < 0.2: # sniper inaccuracy
	...
	if use_mark_overrides:
		...
		radius_override = Data.of("obel1sk.markDetonationRadius")
	...
	cur_arcUpSustain = 0.0
	if not Data.of("obel1sk.arcDirectlyToShot"):
		arc_to_shot(pos_override)
	
	if Data.of("obel1sk.fullAuto") and attack_type == AttackTypes.Shot:
		var incr = Data.of("obel1sk.shootDelayReductionIncrease") / Data.of("obel1sk.burstCount")
		cur_shoot_delay_reduction += incr
		cur_shoot_delay_reduction = clamp(cur_shoot_delay_reduction, 0, Data.of("obel1sk.maxShootDelayReduction"))
	...
	if Data.of("obel1sk.rechargeAmmo") and is_reloading:
		reloadFillAmmo(true, true)
	
	$Reticle.speed_add = Data.of("obel1sk.reticleSpeedAddOnShoot")
	$Reticle.speed_add_duration = Data.of("obel1sk.reticleSpeedAddOnShootDuration")
...
func setCurRestDamageMult(value:float):
	...
	var progress = 0.0
	if Data.of("obel1sk.maxRestDamageMult") != 0:
		progress = cur_restDamageMult / Data.of("obel1sk.maxRestDamageMult")
	$BonusDamageDisplay.set_progress(progress)

func growShootingSpread():
	var _max = Data.of("obel1sk.shootingSpreadMax")
	var growth = Data.of("obel1sk.shootingSpreadGrowth")
	var positive = Data.of("obel1sk.shootingSpreadMax") >= 0 or Data.of("obel1sk.shootingSpreadGrowth") >= 0
	...
	if int(Data.of("obel1sk.chStyle")) != 1:
...
func shot_pos(min_inaccuracy := 0) -> Vector2:
	...
	if $Reticle.cur_spread > Data.of("obel1sk.guaranteedCenterShotThreshold"):
...
func arc_to_shot(shotPosition:Vector2, from:=$ArcOrigin.position):
	...
	if Data.of("obel1sk.arcDirectlyToShot"):
	...
	if Data.of("obel1sk.explosionDelay") > 0:
...
func arc_from_shot(shotPosition:Vector2):
	...
	arc.visual = true
	arc.width = arc_width() / Data.of("obel1sk.burstCount")
	arc.init(from, shotPosition)
	arc.arc(true)
	randomize()
	arc.lifetime = 0.5 + rand_range(-0.15, 0.15)
	
	for i in range(Data.of("obel1sk.shotsAlongArc")):
		var ratio = float(i + 1.0) / float(Data.of("obel1sk.shotsAlongArc") + 1.0)
		...
		var n = preload("res://mods-unpacked/Snek-Phosphorescence/overwrites/content/weapons/obelisk/DeathCandle.tscn").instance()
...
func sky_target(shotPosition:Vector2, obelisk:bool):
	if obelisk:
		var x = $ArcOrigin.global_position.x + shotPosition.x * Data.of("obel1sk.lightningXRatio")
		var y = Data.of("obel1sk.lightningYTarget")
		return Vector2(x, y)
	else: # shot side
		var x = Data.of("obel1sk.lightningXRatio")
		var y = Data.of("obel1sk.lightningYTarget")
		return (Vector2(shotPosition.x * (1.0 - x), shotPosition.y + y))

func arc_width():
	match int(Data.of("obel1sk.attackType")):
		0: #shot
			#return max(Data.of("obel1sk.radius") * Data.of("obel1sk.radius") / 125, 2)
			...
			return 3.5
		1: # beam
			return Data.of("obel1sk.beamWidth")
...
func reloadStart():
	...
	if not Data.of("obel1sk.rechargeAmmo"): $AmmoDisplay.empty_ammo()
	$QuickReload.set_visible(true)
	$QuickReload.show_progress(0.0)
	if not Data.of("obel1sk.rechargeAmmo"): reticle_reload_anim()
...
func reloadTime(includeCurrentAmmo:=false):
	var time = Data.of("obel1sk.reloadTime") * Data.of("obel1sk.reloadTimeMult")
	
	if Data.of("obel1sk.rechargeAmmo") and includeCurrentAmmo:
		var f = 1.0 - ammoRatioOnReloadStart
		time *= f
	
	if $KillstreakTracker.active():
		time *= Data.of("obel1sk.killstreakActiveReloadMultiplier")
	
	if $MercilessTracker.aboveThreshold():
		time *= Data.of("obel1sk.mercilessThresholdReloadMultiplier")
	
	if reloadFromEmpty:
		time *= Data.of("obel1sk.emptyReloadTimeMult")
	
	time -= Data.of("obel1sk.reloadTimeReduction")
	
	return max(time, anticipationDeadzone() + 0.1)

func shootDelay():
	var delay = Data.of("obel1sk.shootDelay")
	
	delay -= cur_shoot_delay_reduction
	
	if $KillstreakTracker.active():
		delay *= Data.of("obel1sk.killstreakActiveShootDelayMultiplier")
		
	if $MercilessTracker.aboveThreshold():
		delay *= Data.of("obel1sk.mercilessThresholdShootDelayMultiplier")
	
	delay -= Data.of("obel1sk.shootDelayReduction")
...
func setStoredDamage(value: int):
	storedDamage = clamp(value, 0, Data.of("obel1sk.storedDamageMax"))
...
func getSpecialName():
	match int(Data.of("obel1sk.specialType")):
		SpecialTypes.Reload:
			if int(Data.of("obel1sk.ammoUsage")) == 0:
				return ""
			else:
				if not Data.of("obel1sk.allowManualReloadStart"):
					return ""
...
func handleMonsterDied(m):
	if Data.of("obel1sk.killstreaks") and not Data.of("obel1sk.killstreakChargeWithDamage"):
		$KillstreakTracker.add_monster_weight(m)
	
	if Data.of("obel1sk.spreadLossOnKill"):
		noSpreadRemainingDuration = Data.of("obel1sk.spreadLossOnKillDuration")
	
	# ammo refill on kill
	if Data.of("obel1sk.ammoRefillOnKill") > 0:
		set_cur_ammo(cur_ammo + max(Data.of("obel1sk.ammoRefillOnKill") * maxAmmo, 1))
	if is_reloading and Data.of("obel1sk.ammoRefillOnKill") > 0.0:
		reloadFillAmmoQuick(false, true)
		set_cur_ammo(cur_ammo+1)
	
	if not Data.of("obel1sk.mercilessGainOnDamage"):
		$MercilessTracker.add_monster_weight(m)
	
	if Data.of("obel1sk.spreadRadiationOnDeath") and m.maxHealth > 10:
		var ra = RAD_AREA.instance()
		Level.stage.add_child(ra)
		ra.global_position = m.getCenter()
	
	if Data.of("obel1sk.deathCandles") and m.techId != "tick" and m.techId != "bigtick":
		var candle = DEATH_CANDLE.instance()
		candle.global_position = m.getCenter()
		candle.lifetime = clamp(float(m.maxHealth) / 10.0, Data.of("obel1sk.deathCandleMinDuration"), Data.of("obel1sk.deathCandleMaxDuration"))
		candle.init()
		$DeathCandleArea.add_child(candle)
	
	remainingAutoAimDuration = Data.of("obel1sk.autoAimDuration")

func handleMonsterDamaged(damage:float):
	if Data.of("obel1sk.killstreaks") and Data.of("obel1sk.killstreakChargeWithDamage"):
		$KillstreakTracker.add_damage_weight(damage)
	
	if Data.of("obel1sk.mercilessGainOnDamage"):
		$MercilessTracker.add_damage_weight(damage)
	
	cur_shootingSpread = max(cur_shootingSpread - Data.of("obel1sk.spreadDecreaseOnHit"), 0)
	$Reticle.shooting_spread = cur_shootingSpread
...
func triggerKillstreak():
	...
	ks.global_position.y = min(pos.y, -10 - Data.of("obel1sk.killstreakEffectRadius") / 2.0)
...
func mercilessMult(bonusType:=""):
	...
	match bonusType:
		"":
			pass
		"damage":
			result *= Data.of("obel1sk.mercilessDamageEfficiency")
			result -= 1.0
		"speed":
			result *= Data.of("obel1sk.mercilessSpeedEfficiency")
	return result + 1.0
...
func canBeamOutlineBeVisible() -> bool:
	if Data.of("obel1sk.chStyle") != 4: # beam
		return false
	return Data.of("obel1sk.radiusOutline") and is_active# and not is_beam_active()
